...
## 12. 官方文案 100% 絕對還原與 1:1 視覺比對紅線 (Copywriting Parity & Visual Inspection)

> **核心紅線**：重構的本質是「**骨架升級與性能現代化**」，絕非「**文案再創作**」。

1. **嚴禁私自篡改、概括或刪減官方文案**：
   - 全站所有頁面文案（包括但不限於：服務介紹、理念說明、關懷大使文案、醫生名銜、收費細則、FAQ 問答、WhatsApp 諮詢指引）必須 **100% 忠實還原線上官網 (`https://smile.hkcmereye.com`)，一字不差**。
   - 杜絕任何未經授權的 AI 文案潤色、總結縮寫或憑空臆造。
2. **語義化重構與 1:1 呈現一一對應**：
   - 舊頁面代碼陳舊低質（如濫用 `div`、混亂的 `nth-child` 嵌套、硬編碼 `padding: 0 160px`），AI Agent 必須主動重構為現代化 HTML5 語義地標（`<main>`, `<section>`, `<article>`, `<figure>`）與清晰的語義化類名。
   - 重構後的最終視覺、排版層次與交互體驗必須與舊頁面 **1:1 精確一一對應**。
3. **主動即時比對驗證機制**：
   - 在開發與修改任何板塊時，必須邊做邊透過瀏覽器截圖比對線上官網 (`https://smile.hkcmereye.com`) 對應頁面，杜絕盲目臆測與偏差累積。

---
*文檔由 Antigravity Agent 自動維護 | 版本：2026.8*
